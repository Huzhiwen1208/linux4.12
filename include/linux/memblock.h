...
struct memblock_region {
	phys_addr_t base; // 内存区域的起始地址
	phys_addr_t size; // 内存区域的大小
	unsigned long flags; // 内存区域的标志
...
struct memblock_type {
	unsigned long cnt; // 内存区域数
	unsigned long max; // 已分配数组的大小
	phys_addr_t total_size;	// 所有内存域总大小
	struct memblock_region *regions; // 内存区域数组
	char *name; // 内存块类型名称
};

struct memblock {
	bool bottom_up; // true: 从低地址向上分配，false: 从高地址向下分配
	phys_addr_t current_limit; // 可分配内存的最大地址
	struct memblock_type memory; // 内存类型（包括已分配内存和未分配内存）
	struct memblock_type reserved; // 预留类型（已分配的内存）
#ifdef CONFIG_HAVE_MEMBLOCK_PHYS_MAP
	struct memblock_type physmem; // 物理内存类型（包含所有内存，是内存类型的超集；内存类型只包含”mem=“配置的范围）
#endif
};
